...
export const TOOL_SCHEMAS = [
  ...
  { name: 'add_accounts', description: 'Add companies the user ALREADY KNOWS as tracked accounts, without AI discovery. Use this whenever you or the user already have the list — from your own research, a spreadsheet, a website, or a conversation. Do NOT route a known list through discover_accounts: that is a tool for FINDING companies, not for adding them, and it will return its own candidates rather than the ones you were asked for. Pass accounts: [{name, domain, region}] — include the domain wherever you know it, since it is what makes contact scouting work later. Each domain is checked by fetching the site, but a failed check does NOT block: unverified ones are still added and reported back under `unverified` for the user to glance at. Optionally creates a SAMpaign: campaign_mode "list" (default) makes ONE campaign covering all of them, which is right for one message to many companies; "one_per_account" makes one each. Does not scout contacts and sends nothing. Managers and above. Max 50 per call.', params: { accounts: { type: 'array', required: true }, campaign_name: { type: 'string' }, campaign_goal: { type: 'string' }, campaign_mode: { type: 'string', enum: ['list','one_per_account','single'] } } },
