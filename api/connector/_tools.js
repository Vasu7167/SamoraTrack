...
export const TOOL_SCHEMAS = [
  ...
  { name: 'discover_accounts', description: 'TOP OF FUNNEL. Find NEW companies to sell to that are not yet in the pipeline, from a plain-English description ("private universities in India with liberal arts departments", "FMCG distributors in the Gulf"). This PROPOSES ONLY: it creates nothing, spends no enrichment credits, and sends nothing. Each candidate is checked by fetching its website, so `verified: true` means the domain is live and its homepage actually names the organisation, and `verified: false` with the evidence line tells you why not. ALWAYS show the user the candidates and their evidence, and ALWAYS let them choose, before calling commit_discovery. Never accept unverified candidates on the user\'s behalf. Requires Discovery Mode to be enabled for the org and a manager-or-above role. Cap is 50 per call; prefer 10 to 25 so the user can actually read the list. You do NOT need to call get_company_context first: this tool reads the org\'s products and ICP definition itself and gives them to the search, and reports back in `context_used` what it knew. If `context_note` says no products or ICP are configured, tell the user, because that is why the results feel generic.', params: { description: { type: 'string', required: true }, region_hint: { type: 'string' }, limit: { type: 'number' } } },
  { name: 'commit_discovery', description: 'Turn candidates the USER HAS EXPLICITLY ACCEPTED into real accounts. Call ONLY after discover_accounts and only with ids the user chose — never auto-accept everything you found. accept_ids become tracked accounts; reject_ids are remembered so the same suggestion is not proposed again. If you pass campaign_name, it creates ONE SAMPAIGN PER ACCEPTED ACCOUNT, named "<campaign_name> — <account>", because a SAMpaign is anchored to exactly one account and that is what makes its scouted contacts provably belong to it. Accepting 20 accounts therefore creates 20 campaigns: tell the user that before you call it. Pass campaign_mode:"single" only if they explicitly want one campaign on the first account. campaign_goal is the pitch/ask and is copied to every campaign created. This does NOT scout contacts and does NOT schedule or send anything: after it returns, tell the user to run contact scouting when ready. Duplicates against existing accounts are detected and skipped, and reported back.', params: { accept_ids: { type: 'array', required: true }, reject_ids: { type: 'array' }, campaign_name: { type: 'string' }, campaign_goal: { type: 'string' }, campaign_mode: { type: 'string', enum: ['one_per_account','single'] } } },
  ...
  switch (name) {
    ...
    case 'commit_discovery': {
      const d = await edge(accessToken, 'commit_discovery', {
        ...
        campaign_mode: args.campaign_mode || 'one_per_account'
      });
      return d;
    }
