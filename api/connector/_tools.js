...
export const TOOL_SCHEMAS = [
  ...
  { name: 'scout_list_accounts', description: 'Scout contacts for EVERY company in a list SAMpaign, in one go, using the campaign\'s "Who to hunt" targets. This is the natural next step after add_accounts or commit_discovery: offer it as soon as a list is created rather than making the user scout companies one at a time. It costs enrichment credits, so confirm with the user first, then call it. It is RESUMABLE: it does a few accounts per call, skips ones already scouted, and returns `remaining` — keep calling with the same campaign_id until remaining is 0, and tell the user the progress as you go. Reports per account how many contacts were found, how many have a real email, and how many need enrichment. An account with no domain is reported as such rather than as an empty result.', params: { campaign_id: { type: 'string', required: true }, account_ids: { type: 'array' }, batch: { type: 'number' } } },
  { name: 'add_accounts', description: 'Add companies the user ALREADY KNOWS as tracked accounts, without AI discovery. Use this whenever you or the user already have the list — from your own research, a spreadsheet, a website, or a conversation. Do NOT route a known list through discover_accounts: that is a tool for FINDING companies, not for adding them, and it will return its own candidates rather than the ones you were asked for. Pass accounts: [{name, domain, region}] — include the domain wherever you know it, since it is what makes contact scouting work later. Each domain is checked by fetching the site, but a failed check does NOT block: unverified ones are still added and reported back under `unverified` for the user to glance at. Optionally creates a SAMpaign: campaign_mode "list" (default) makes ONE campaign covering all of them, which is right for one message to many companies; "one_per_account" makes one each. Does not scout contacts and sends nothing, but you should OFFER to scout next: after this returns, tell the user how many accounts were created and ask whether to run scout_list_accounts on the new list. Managers and above. Max 50 per call.', params: { accounts: { type: 'array', required: true }, campaign_name: { type: 'string' }, campaign_goal: { type: 'string' }, campaign_mode: { type: 'string', enum: ['list','one_per_account','single'] } } },
...
export async function executeTool(accessToken, name, args = {}) {
  switch (name) {
    ...
    case 'scout_list_accounts':
      return edge(accessToken, 'scout_list_accounts', { campaign_id: args.campaign_id, account_ids: args.account_ids || null, batch: args.batch || 3 });
    case 'add_accounts':
